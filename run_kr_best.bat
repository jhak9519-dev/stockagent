@echo off
REM 한국장 마감 후 실행용: 한국 주식 중 최유망 1종목 리포트 (스케줄러가 호출)
chcp 65001 >nul
cd /d "%~dp0"
...
echo ===================== KR best  %date% %time% =====================>> "logs\scheduler_kr.log"
"%~dp0.venv\Scripts\python.exe" run.py --best --market KR >> "logs\scheduler_kr.log" 2>&1
set "GEN_ERR=%errorlevel%"
if not "%GEN_ERR%"=="0" echo [!] 리포트 생성 실패 (exit=%GEN_ERR%) - AI 엔진/네트워크/로그인 점검 필요>> "logs\scheduler_kr.log"
REM 리포트 생성 직후 자동 점검(수치 검산·구조·폴백·오늘자 리포트 유무)
"%~dp0.venv\Scripts\python.exe" tools\daily_review.py >> "logs\scheduler_kr.log" 2>&1
